...
def tf_inverse(q, t):
    ...
    return q_inv, -quat_rotate(q_inv, t)


@torch.jit.script
def tf_apply(q, t, v):
    return quat_rotate(q, v) + t
...
def tf_combine(q1, t1, q2, t2):
    return quat_mul(q1, q2), quat_rotate(q1, t2) + t1
...
def get_euler_xyz(q):
    ...
    cosy_cosp = qw2 + qx2 - qy2 - qz2
    yaw = torch.atan2(siny_cosp, cosy_cosp)

    pix2 = 2 * np.pi
    return roll % pix2, pitch % pix2, yaw % pix2
...
def local_to_world_space(pos_offset_local: torch.Tensor, pose_global: torch.Tensor):
    ...
    result_pos_gloal = pos_offset_global + pose_global[:, 4:7]  #+ pose_global[:, 0:3]
