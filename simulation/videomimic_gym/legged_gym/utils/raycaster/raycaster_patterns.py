...
if TYPE_CHECKING:
    from . import raycaster_cfg

def grid_pattern(cfg: raycaster_cfg.GridPatternCfg, device: str) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    nx = int(cfg.size[0] / cfg.resolution)
    ny = int(cfg.size[1] / cfg.resolution)

    # Create grid points from -size/2 to +size/2
    x = torch.linspace(-cfg.size[0]/2, cfg.size[0]/2, nx + 1, device=device)
    y = torch.linspace(-cfg.size[1]/2, cfg.size[1]/2, ny + 1, device=device)
...
def pinhole_camera_pattern(
    cfg: raycaster_cfg.PinholeCameraPatternCfg, intrinsic_matrices: torch.Tensor, device: str
    ...
    # Ensure intrinsic matrices are on the correct device
...
def bpearl_pattern(cfg: raycaster_cfg.BpearlPatternCfg, device: str) -> tuple[torch.Tensor, torch.Tensor]:
...
def lidar_pattern(cfg: raycaster_cfg.LidarPatternCfg, device: str) -> tuple[torch.Tensor, torch.Tensor]:
